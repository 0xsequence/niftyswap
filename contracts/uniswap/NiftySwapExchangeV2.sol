 ...
 * on 'Base Tokens'.
 */
contract NiftySwapExchangeV2 is ERC1155Metadata, ERC1155MintBurn, ERC1155Meta {
  ...
  //  *  [] Return values?
  //  *  [] Add single token onReceive function
  //  */
  ...
  //   assembly {
  //     functionSignature := and(_data, 0xFFFFFFFF00000000000000000000000000000000000000000000000000000000)
  //   }
  ...
  //     require(msg.sender == address(baseToken), "NiftySwapExchange#onERC1155Received: INVALID_BASE_TOKENS_TRANSFERRED");
  //     require(_ids.length == 1, "NiftySwapExchange#onERC1155Received: INVALID_BASE_TOKEN_ID_AMOUNT");
  //     require(_ids[0] == baseTokenID, "NiftySwapExchange#onERC1155Received: INVALID_BASE_TOKEN_ID");
  ...
  //     _baseToToken(obj.tokensBoughtIDs, obj.tokensBoughtAmounts, obj.maxBaseTokens, _amounts[0], obj.deadline, _from);
  ...
  //     // Tokens received need to be Token contract
  //     require(msg.sender == address(token), "NiftySwapExchange#onERC1155Received: INVALID_TOKENS_TRANSFERRED");
  ...
  //     require(msg.sender == address(token), "NiftySwapExchange#onERC1155Received: INVALID_TOKEN_TRANSFERRED");
  ...
  //     require(msg.sender == address(this), "NiftySwapExchange#onERC1155Received: INVALID_UNI_TOKENS_TRANSFERRED");
  ...
  //   } else {
  //     revert("NiftySwapExchange#onERC1155Received: INVALID_METHOD");
  //   }
  ...
  //  * @dev If _baseTokensAmount is lower than sum(_maxBaseTokens), the whole tx will fail
  //  * @dev Will skip IDs with invalid amount or where cost exceeds their _maxBaseToken limit
  ...
  //  * @param _maxBaseTokens    Array of maximum amount of base tokens to spend for each corresponding Token id in _tokenIds
  //  * @param _baseTokensAmount Total amount of Base Tokens received
  ...
  //   require(nTokens == _maxBaseTokens.length, "UniswapExchange#_baseToToken: INVALID_LENGTH_FOR_MAXBASETOKENS");

  //   // Initialize variables
  //   uint256 totalRefundBaseTokens = _baseTokensAmount; // Total amount of Base Tokens to refund
  //   uint256[] memory tokenIds;                         // Token IDs actually purchased
  //   uint256[] memory tokenAmounts;                     // Amount of tokens actually purchased
  //   uint256[] memory baseTokensSold;                   // Amount of base tokens sold per ID
  ...
  //     uint256 maxBaseTokens = _maxBaseTokens[i];
  ...
  //     uint256 baseTokenAmount = getBuyPrice(amountBought, baseReserve.sub(maxBaseTokens), tokenReserve);

  //     // Check if cost is higher than limit provided
  //     if (baseTokenAmount > maxBaseTokens) {
  //       // require(maxBaseTokens >= baseTokenAmount, "UniswapExchange#_baseToToken: PRICE_EXCEEDS_MAXIMUM");
  //       continue;
  //     }
  ...
  //     tokenIds.push(idBought);
  //     tokenAmounts.push(amountBought);
  //     baseTokensSold.push(baseTokenAmount);

  //     // Update individual base reseve amount
  //     baseTokenReserve[idBought] = baseTokenReserve[idBought].add(baseTokenAmount);
  //   }
  ...
  //   token.safeBatchTransferFrom(address(this), _recipient, tokenIds, tokenAmounts, "");
  //   emit TokensPurchase(_recipient, tokenIds, tokenAmounts, baseTokensSold);
  // }
  ...
  //  * @param _id           Token ID that is sold
  //  * @param _tokensSold   Amount of Tokens _id sold.
  //  * @param _minBaseToken Minimum amount of Base Tokens to receive
  //  * @param _deadline     Block number after which this transaction can no longer be executed.
  ...
  //   require(nTokens == _minBaseTokens.length, "UniswapExchange#_tokenToBase: INVALID_LENGTH_FOR_MINBASETOKENS");

  //   // Initialize variables
  //   uint256 totalBaseTokens = 0;      // Total amount of Base tokens to transfer
  //   uint256[] memory tokenIds;        // Token IDs actually purchased
  //   uint256[] memory tokenAmounts;    // Amount of tokens actually purchased
  //   uint256[] memory baseTokensBought;  // Amount of base tokens sold per ID
  //   uint256[] memory idsToRefund;     // Array of token id to refund
  //   uint256[] memory amountsToRefund; // Array of amounts to refund for corresponding ID
  ...
  //     uint256 minBaseTokens = _minBaseTokens[i];

  //     // If 0 tokens send for this ID, skip
  //     if (amountSold == 0) {
  //       //require(_tokensSold >= 0, "UniswapExchange#_tokenToBase: NULL_TOKENS_SOLD");
  //       continue;
  //     }
  ...
  //     uint256 baseTokenAmount = getSellPrice(amountSold, tokenReserve, baseReserve);

  //     if (minBaseTokens > baseTokenAmount) {
  //       // Store information of tokens to refund
  //       idsToRefund.push(idSold);
  //       amountsToRefund.push(amountSold);

  //       //require(baseTokenBought >= _minBaseToken, "UniswapExchange#_tokenToBase: INSUFFICIENT_BASE_TOKENS_RECEIVED");
  //       continue; // Skip
  //     }
  ...
  //     // Append Token id, Token id amount and Base Token amount to tracking arrays
  //     tokenIds.push(idSold);
  //     tokenAmounts.push(amountSold);
  //     baseTokensBought.push(baseTokenAmount);
  //   }

  //   // Transfer baseTokens here
  //   baseToken.safeTransferFrom(address(this), _recipient, baseTokenID, totalBaseTokens, "");

  //   // Refund tokens when sale failed
  //   if (idsToRefund.length > 0) {
  //     token.safeBatchTransferFrom(address(this), _recipient, idsToRefund, amountsToRefund, "");
  //   }

  //   // Might need to emit in onReceive
  //   emit BaseTokenPurchase(_recipient, tokenIds, tokenAmounts, baseTokensBought);
  // }
  ...
  //       uint256 liquidityMinted = baseTokenAmount.mul(totalLiquidity) / baseReserve;

  //       // Mint liquidity ownership tokens and increase liquidity supply accordingly
  //       _mint(_provider, id, liquidityMinted);
  //       totalSupplies[id] = totalLiquidity.add(liquidityMinted);
  ...
  //       _mint(_provider, id, maxBaseToken);
  ...
  //   baseToken.safeTransferFrom(msg.sender, address(this), baseTokenID, totalBaseTokens, abi.encodePacked(DEPOSIT_SIG));
  // }
  ...
  //   require(nTokens == _UNItokenAmounts.length, "UniswapExchange#_removeLiquidity: INVALID_LENGTH_FOR_IDS_AMOUNTS");
  //   require(nTokens == _minBaseTokens.length, "UniswapExchange#_removeLiquidity: INVALID_LENGTH_FOR_MINBASETOKENS");
  //   require(nTokens == _minTokens.length, "UniswapExchange#_removeLiquidity: INVALID_LENGTH_FOR__MINTOKENS");
  ...
  //     require(_minTokens[i] > 0, "UniswapExchange#_removeLiquidity: INVALID_MIN_TOKENS");
  //     require(_minBaseTokens[i] > 0, "UniswapExchange#_removeLiquidity: INVALID_MIN_BASE_TOKENS");
  ...
  //     emit RemoveLiquidity(_provider, baseTokenAmount, tokenAmount);
  //   }
