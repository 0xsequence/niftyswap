...
import {IERC20Wrapper} from "@0xsequence/erc20-meta-token/contracts/interfaces/IERC20Wrapper.sol";
...
contract WrapAndNiftyswap {
    IERC20Wrapper public immutable tokenWrapper; // ERC-20 to ERC-1155 token wrapper contract
    address public immutable exchange; // Niftyswap exchange to use
    address public immutable erc20; // ERC-20 used in niftyswap exchange
    address public immutable erc1155; // ERC-1155 used in niftyswap exchange

    uint256 internal immutable wrappedTokenID; // ID of the wrapped token
    ...
    function wrapAndSwap(uint256 _maxAmount, address _recipient, bytes calldata _niftyswapOrder) external {
        ...
        IERC1155(erc1155).safeBatchTransferFrom(
            address(this), _recipient, obj.tokensBoughtIDs, obj.tokensBoughtAmounts, ""
        );
    }
